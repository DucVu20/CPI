...
#define HREF_VSYNC_PCLK         (CPI_BASE_ADDR + 0x2C)
...
void software_check_resolution(){
  int height, width;
  while(!(reg_read8(HREF_VSYNC_PCLK) & 0x02));
  while((reg_read8(HREF_VSYNC_PCLK) & 0x02));
  
  while(!(reg_read8(HREF_VSYNC_PCLK) & 0x02)) {
    while(!(reg_read8(HREF_VSYNC_PCLK) & 0x04)); // wait for href to go high
    while((reg_read8(HREF_VSYNC_PCLK) & 0x04)){ // while vsync is true
      while(!(reg_read8(HREF_VSYNC_PCLK) & 0x01)); // wait for pclk to go high
      width = width + 1;
      while((reg_read8(HREF_VSYNC_PCLK) & 0x01)); // wait for pclk to go low
    }
    height = height + 1;
  }
  printf("the resolution count by software height: %08d", height);
  printf(", width: %08d\n", width);
  printf("\n");
}
...
int main(void){
  ...
  reg_write8(XCLK_PRESCALER, 6); // generate 25 MHz XCLK for the camera
  ...
  printf(".........configure QVGA: 320x240........\n");
  configure_camera(REG_COM7, COM7_FMT_QVGA);
  for(int a =0; a<10; a++){ // capture 10 frames
    capture_image();
    while((reg_read8(INTERFACE_STATUS) & 0x02) == 0); // wait for new frame
    printf("frame %d: ", a);
