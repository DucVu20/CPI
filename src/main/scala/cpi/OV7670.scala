...
class CPI(val img_width: Int,val img_height: Int,
          val byte_per_pixel: Int, val CLK_FREQ_MHz: Float,
          val SCCB_FREQ_kHz: Float) extends Module with HasCPIIO {
  val capture_module=Module(new capture_module(img_width,img_height,
                                            byte_per_pixel))
