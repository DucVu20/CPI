...
class vbp_uint(NUM_BIT_IN      : Int,
  ...
  val io = IO(new Bundle {
    ...
    val a        : Vec[UInt] = Input(Vec(NUM_CONCURRENT, UInt(BIT_WIDTH_A.W)))
  ...
  val a_reg : Vec[UInt] = RegInit(VecInit(Seq.fill(NUM_CONCURRENT)(0.U(BIT_WIDTH_A.W))))
  ...
  for (i <- 0 until NUM_CONCURRENT) {
    when(io.start_in === 1.U) {
      ...
      when (ops(i) === 1.U) {
        partial_sum(i) := io.w(i)
      }.otherwise {
        partial_sum(i) := 0.S
      }
    }.otherwise {
      a_reg(i) := (a_reg(i) >> 1).asUInt()
      ops(i)   := a_reg(i)(1)

      when (ops(i) === 1.U) {
        partial_sum(i) := w_reg(i)
      }.otherwise {
        partial_sum(i) := 0.S
      }
    }
